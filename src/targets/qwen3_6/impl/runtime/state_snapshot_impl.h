...
std::vector<std::string> snapshot_aliases(const std::vector<TokenId>& tokens,
    ...
    for (const auto fingerprint : identity.persistent_fingerprints(tokens, minimum_frontier)) {
        aliases.push_back(snapshot_alias(fingerprint.frontier, fingerprint.hash));
    }
    return aliases;
}

std::string snapshot_alias_at(const std::vector<std::string>& aliases, std::uint32_t frontier) {
    const std::string prefix = std::to_string(frontier) + '-';
    const auto found = std::find_if(aliases.begin(), aliases.end(), [&](const std::string& alias) {
        return alias.starts_with(prefix);
    });
    if (found == aliases.end()) {
        throw std::logic_error("snapshot frontier divides a Vision item");
    }
    return *found;
}
...
std::vector<Tensor> snapshot_tensors(ProgramImplCore& p, std::uint32_t lane, bool checkpoint) {
    ...
    for (const auto slot :
         {LinearStateSlots::current_state_slot(lane, p.max_concurrency),
    ...
    for (auto* cache : {&p.decoder->text_kv, p.decoder->mtp_cache()}) {
        ...
        for (std::uint32_t layer = 0; layer < cache->layers(); ++layer) {
            ...
            if (checkpoint) {
                const auto checkpoint_tail = cache->kvarn_rewrite_checkpoint_tail_view(layer, lane);
...
std::shared_ptr<runtime::StateSnapshotImage> capture_image(ProgramImplCore& p, std::uint32_t lane,
    ...
    const auto aliases = snapshot_aliases(s.ledger, s.prefix_identity);
    image->key = std::move(key);
    if (s.execution_frontier) {
        image->aliases.push_back(snapshot_alias_at(aliases, s.execution_frontier));
    }
    if (s.rewrite_checkpoint.valid) {
        const std::string checkpoint = snapshot_alias_at(aliases, s.rewrite_checkpoint.frontier);
        if (image->aliases.empty() || image->aliases.front() != checkpoint) {
            image->aliases.push_back(checkpoint);
        }
    }
    ...
    SnapshotJson j{{"version", kStateSnapshotAbi},
                   ...
                   {"mtp_valid", s.mtp_kv_valid},
                   {"rope_delta", s.rope_delta},
                   {"checkpoint", s.rewrite_checkpoint.valid},
                   {"checkpoint_kind", static_cast<int>(s.rewrite_checkpoint.kind)},
                   {"checkpoint_frontier", s.rewrite_checkpoint.frontier},
                   {"tail_valid", s.tail_hidden_valid},
                   {"ledger", s.ledger},
                   {"identity", SnapshotJson::binary(s.prefix_identity.serialize())},
    ...
    p.decoder->text_kv.snapshot_to_host(
        s.kv->text, std::span<std::uint8_t>(image->payload).subspan(offset, main_bytes),
        p.device.stream);
    offset += main_bytes;
    if (s.kv->backend)
        p.decoder->mtp_cache()->snapshot_to_host(
            *s.kv->backend, std::span<std::uint8_t>(image->payload).subspan(offset),
            p.device.stream);
...
void ProgramImplCore::configure_state_cache(const EngineOptions& options) {
    ...
    SnapshotJson identity{{"state_format", kStateSnapshotAbi},
                          ...
                          {"spec", static_cast<int>(speculative_backend)},
                          {"draft", draft_window},
                          {"proposal", static_cast<int>(proposal_head)},
                          {"prefill_chunk", prefill_chunk},
                          {"vision", options.enable_vision},
                          {"style", static_cast<int>(options.chat_style)},
                          {"template", options.chat_template_override}};
    state_cache = std::make_unique<runtime::StateSnapshotCache>(
        options.state_cache_dir, options.state_cache_max_bytes, options.state_cache_ram_bytes,
        identity.dump());
}

runtime::StateSnapshotLoad ProgramImplCore::lookup_state(const PreparedPromptData& prompt,
                                                         std::uint32_t minimum_frontier) {
    if (!state_cache || !prompt.identity.reusable) return {};
    qwen3_6::detail::ResidentPrefixIdentity identity;
    identity.assign(prompt);
    auto aliases = snapshot_aliases(prompt.token_ids, identity, minimum_frontier);
...
void ProgramImplCore::save_state(std::uint32_t lane) noexcept {
    if (!state_cache || !has_retained_lane(lane) || !sequences[lane].kv ||
        sequences[lane].ledger.size() < 256)
        return; // avoid fixed GDN snapshot cost for tiny/warmup requests
    try {
        const auto aliases =
            snapshot_aliases(sequences[lane].ledger, sequences[lane].prefix_identity);
        if (aliases.empty()) return;
        const std::string key = runtime::state_hash_key(aliases.back());
...
bool ProgramImplCore::restore_state(std::uint32_t lane, const PreparedPromptData& prompt,
    ...
    if (!state_cache || lane >= max_concurrency || requests[lane].lifecycle == Lifecycle::Active ||
        requests[lane].lifecycle == Lifecycle::Pending ||
    ...
    try {
        ...
        if (ledger.empty() || ledger.size() > capacity + 1ULL ||
            ledger.size() != j.at("ledger_frontier") || execution > text_valid ||
            text_valid > capacity || mtp_valid > text_valid || execution > ledger.size() ||
            main_pages != pages_for_tokens(text_valid) ||
            mtp_pages != pages_for_tokens(mtp_valid) || main_pages == 0 ||
            (speculative_backend == SpeculativeBackend::Mtp) != (mtp_pages != 0) ||
            (checkpoint && (checkpoint_frontier == 0 || checkpoint_frontier > text_valid ||
                            kind < 0 || kind > 1)))
            return false;
        qwen3_6::detail::ResidentPrefixIdentity identity;
        ...
        const bool append =
            execution && qwen3_6::detail::prefix_matches(prompt, ledger, identity, execution);
        const bool rewrite = checkpoint && qwen3_6::detail::prefix_matches(prompt, ledger, identity,
                                                                           checkpoint_frontier);
        ...
        decoder->text_kv.restore_from_host(
            s.kv->text, std::span<const std::uint8_t>(image.payload).subspan(offset, main_bytes),
            device.stream);
        offset += main_bytes;
        if (mtp_pages)
            decoder->mtp_cache()->restore_from_host(
                *s.kv->backend, std::span<const std::uint8_t>(image.payload).subspan(offset),
                device.stream);
        device.synchronize();
        s.ledger                 = std::move(ledger);
        s.prefix_identity        = std::move(identity);
        s.execution_frontier     = execution;
        s.ledger_frontier        = s.ledger.size();
        s.text_kv_valid          = text_valid;
        s.mtp_kv_valid           = mtp_valid;
        s.rope_delta = j.at("rope_delta").get<std::int32_t>();
        s.rewrite_checkpoint     = {checkpoint, static_cast<RewriteCheckpointKind>(kind),
                                    checkpoint_frontier};
        s.tail_hidden_valid      = j.at("tail_valid").get<bool>();
        s.mtp_draft_count        = 0;
        s.retained               = true;
        ...
        if (std::getenv("NINFER_VERIFY_STATE_RESTORE")) {
            const auto aliases =
                snapshot_aliases(sequences[lane].ledger, sequences[lane].prefix_identity);
            if (aliases.empty()) { throw std::runtime_error("restored state has no snapshot key"); }
            const auto actual = capture_image(*this, lane, runtime::state_hash_key(aliases.back()));
