...
void mtp_prepare_next_round_launch(const Tensor& verify_ids, const Tensor& next_anchors,
                                   ...
                                   std::int32_t max_context, std::int32_t speculative_page_size,
                                   std::int32_t speculative_sink_tokens, cudaStream_t stream) {
    ...
    mtp_prepare_next_round_kernel<<<grid, kBlock, 0, stream>>>(
        ...
        speculative_page_size, speculative_sink_tokens);
