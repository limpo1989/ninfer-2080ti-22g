...
KvCacheStorage parse_kv_cache(std::string_view text) {
    ...
    if (text == "kvarn") { return KvCacheStorage::KvarnK4V2; }
    if (text == "kvarn-k4v4") { return KvCacheStorage::KvarnK4V4; }
    throw std::invalid_argument("--kv-dtype must be bf16, int8, kvarn, or kvarn-k4v4");
}
...
std::string usage_text(std::string_view program) {
    ...
    out << "Usage: " << program << " --weights <artifact.ninfer> [options]\n\n"
        ...
        << "  --kv-dtype <bf16|int8|kvarn|kvarn-k4v4> KV cache storage (default: bf16)\n"
    ...
    for (std::size_t i = 0; i < results.size(); ++i) {
        ...
        for (std::size_t r = 0; r < result.reps.size(); ++r) {
            ...
            out << "        {\n"
                ...
                << "          \"generated_token_hash\": \"" << rep.generated_token_hash << "\",\n"
...
std::string kv_cache_name(KvCacheStorage storage) {
    ...
    case KvCacheStorage::Int8Group64:
        return "int8-group64";
    case KvCacheStorage::KvarnK4V2:
        return "kvarn";
    case KvCacheStorage::KvarnK4V4:
        return "kvarn-k4v4";
    }
    return "unknown";
}
