...
| `throughput` | interval token deltas and rates, scheduler occupancy, active prefill/decode progress, and decode-round batch statistics |
...
reused prefix tokens, and computed tokens committed at completed chunk boundaries. While decode is
active, `decode_progress` reports one decode-ready request's Engine request ID, prompt token count,
generated token count (including the first token licensed by prefill), and effective output limit.
The deployment dashboard renders computed/required progress, a 30-second rolling rate, and ETA for
both phases. An older running server without these schema-v13 fields still produces the current
interval rate but cannot provide an exact percentage or ETA.
